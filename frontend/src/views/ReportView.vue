...
const createCoverPage = (pdf: jsPDF): void => {
  ...
  const title = t("message.report.report_title");
  ...
  const subtitle = t("message.report.report_subtitle");
...
const addInsightsToPdf = (pdf: jsPDF, key: string, startY: number): number => {
  ...
  if (chartInsights.value[key] && chartInsights.value[key].length > 0) {
    pdf.setFontSize(16);
    pdf.setTextColor(51, 51, 51);
    pdf.text(`${t("message.report.key_insights")}`, 20, currentY);
    currentY += 10;
...
const addSummaryPage = (pdf: jsPDF): void => {
  ...
  pdf.setTextColor(123, 7, 121);
  pdf.setFont('helvetica', 'bold');
  pdf.text(`${t("message.report.summary_text")}`, width / 2, 30, { align: 'center' });
  ...
  pdf.setFontSize(11);
  pdf.text(`${t("message.report.report_info")}`, 30, y);
  y += 7;
  pdf.text(`${t("message.report.visualization_info")}`, 30, y);
  y += 7;
  pdf.text(`${t("message.report.decision_info")}`, 30, y);
...
<template>
  <main class="analytics-container">
    <div class="header-section">
      <h1 class="title">{{t('message.report.title')}}</h1>
      <p class="subtitle"></p>
    </div>

    <div class="selection-panel">
      <div class="selection-panel-inner">
        <h5 class="panel-title">{{t('message.report.panel_title')}}</h5>

        <div v-if="isLoadingStructure" class="loading-state">
          <div class="spinner">
            <div class="spinner-inner"></div>
          </div>
          <p>{{t('message.report.loading_structure')}}</p>
        </div>

        <template v-else>
          <div class="tables-section">
            <h6 class="section-subtitle">{{t('message.report.table_selection')}}</h6>
          ...
          <div class="selection-summary" v-if="Object.keys(selectedTableColumns).length > 0">
            <h6>{{t("message.report.selected_items")}}</h6>
          ...
          <div class="button-group">
            ...
            >
              ...
              <span v-else class="btn-text">{{t('message.report.generate_charts')}}</span>
            </button>
            ...
            >
              ...
              <span v-else class="btn-text">{{t('message.report.refresh_schema')}}</span>
            </button>
            ...
            >
              ...
              <span v-else class="btn-text">
                ...
                {{t('message.report.export_pdf')}}
              </span>
            </button>
          </div>

          <div v-if="!hasSelectedItems" class="selection-hint">
            {{t('message.report.selection_info')}}
          </div>
        </template>
        ...
        <div class="report-header">
          ...
          <div v-if="chartInsights[key] && chartInsights[key].length > 0" class="insights-panel">
            <div class="insights-header" @click="toggleDetails(key)">
              <h3>{{t('message.report.key_insights')}}</h3>
    ...
    <div v-if="isExportingPdf" class="pdf-export-overlay">
      <div class="pdf-export-modal">
        ...
        <h3>{{t('message.report.generating_pdf')}}</h3>
