...
def get_db_structure(query_service: QueryService = Depends(get_query_service)):
    try :
        ...
        return ResponseManager.success_response(data={"db_structure": db_structure}, message="Success", status_code=status.HTTP_200_OK)
    except Exception as e:
        return ResponseManager.error_response(message="Error", status_code=status.HTTP_400_BAD_REQUEST, details={"error": str(e)})
